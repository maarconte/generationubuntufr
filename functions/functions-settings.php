...
function gur_enqueue() {
  ...
  // scripts
  wp_register_script('gur_/scripts', get_template_directory_uri() . '/script.min.js', false, array( 'jquery' ), true);
  wp_enqueue_script('gur_/scripts');
  // styles
  wp_enqueue_style('gur_/styles', get_template_directory_uri() . gur_get_cachebusted_css(), false, null);
  // Typekit
  global $typekit_id;
  if ($typekit_id) :
...
add_action('wp_enqueue_scripts', 'gur_enqueue');


/* 1.2 THEME SUPPORT
/––––––––––––––––––––––––*/
function gur_theme_support()  {
...
add_action( 'after_setup_theme', 'gur_theme_support');
...
add_action('after_setup_theme', 'gur_wphead_cleanup');


/* 2.2 LOAG OG-TAGS
/––––––––––––––––––––––––*/
// loads open graph tags » http://ogp.me/
// use 'gur_load_ogtags(true)' to also display the og:image tag
function gur_ogtags() {
  echo '
...
function gur_get_cachebusted_css() {
