...
<?php $fond = get_sub_field('fond'); ?>
 <section class="section section-stats <? echo $fond == "Couleur" ? "bg-primary": $fond == "Gris" ? "bg-light" : "" ?>">
    ...
    <!-- Section background: image -->
     <div class="container">
        ...
        <?php if (have_rows('radial_chart') ) : $i = 1 ; ?>
            ...
            <div class="col-xs-6 col-sm-6 col-md-3 circle-bar circle-<?= $i ?>">
                <svg class="radial-progress" data-percentage=" <?php if (get_sub_field('percentage') ) : ?> <?= get_sub_field('percentage'); ?><?php endif; ?>" viewBox="0 0 80 80">
                    ...
                    <text class="percentage text-circle-1" x="50%" y="57%" transform="matrix(0, 1, -1, 0, 80, 0)">
                                <?php if (get_sub_field('operateur') ) : ?>
                                    <?= get_sub_field('operateur'); ?>
                                <?php endif; ?>
                                <?php if (get_sub_field('percentage') ) : ?>
                                    <?= get_sub_field('percentage'); ?>
                                <?php endif; ?>
                    </text>
                </svg>
                <?php if (get_sub_field('label') ) : ?>
                    <h5 class="circle-bar-title"><?= get_sub_field('label'); ?></h5>
                <?php endif; ?>
            </div>
            <?php $i++; endwhile ?>
        <?php endif ?>
